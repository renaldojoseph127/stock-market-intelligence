...
import { getBatchReportIssuePage } from "@/lib/import/batch-results";
...
export async function GET(
  ...
  try {
    ...
    return NextResponse.json(
      await getBatchReportIssuePage(db, id, reportId, pagination),
      { headers: { "cache-control": "no-store" } },
    );
  } catch (error) {
    return NextResponse.json(
      { error: error instanceof Error ? error.message : "Could not load report issues." },
      { status: 400 },
    );
  }
}
