...
export async function GET(
  ...
  try {
    const brief = await assembleTickerResearchBrief(db, decodeURIComponent(symbol), dataMode);
    if (!brief) return NextResponse.json({ message: "Ticker not found." }, { status: 404 });
    ...
    return new Response(researchBriefHTML(brief), {
