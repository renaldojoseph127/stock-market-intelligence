...
export class HistoricalDataQualityEngine {
  ...
  detectPossibleDecimalLoss(input: QualityAppearanceInput): QualityFindingDraft[] {
    ...
    const evidence = { localMedian: baseline, localObservationCount: this.priceNeighbors(input.neighbors ?? []).length, currentToMedianRatio: input.price / baseline, impliedPriceFromDollarVolume: implied, candidates, rawPriceToken: raw, sourceLine: input.rawValues?.line ?? null, lookAheadPolicy: "prior_observations_only", asOfDate: input.reportDate, ambiguity: "Decimal placement is inferred; source image/manual review remains required." };
    return [{ fieldName: "price", findingType: "possible_missing_decimal", severity: input.price / baseline >= 50 ? "critical" : "high", originalValue: textValue(input.price), numericOriginalValue: input.price, ruleId: "price_missing_decimal_v1", ruleVersion: this.ruleVersion, confidenceScore: confidence, evidence,
      proposal: this.createProposal({ proposedValue: String(best.value), proposedNumericValue: best.value, proposalMethod: "decimal_restoration", confidenceScore: confidence, reason: `Restoring ${Math.log10(best.scale)} decimal place(s) best matches the robust prior-only median${implied == null ? "" : " and same-row dollar-volume-implied price"}. Human approval is required.`, evidence: { selectedScale: best.scale, localError: best.localError, impliedError: best.impliedError, lookAheadPolicy: "prior_observations_only", asOfDate: input.reportDate, resolutionEngineVersion: "data-quality-resolution-v1", resolutionWarnings: [] } }) }];
  }
  ...
  detectPossibleExtraDecimal(input: QualityAppearanceInput): QualityFindingDraft[] {
    ...
    return [{ fieldName: "price", findingType: "possible_extra_decimal", severity: "medium", originalValue: textValue(input.price), numericOriginalValue: input.price, ruleId: "price_extra_decimal_v1", ruleVersion: this.ruleVersion, confidenceScore: confidence, evidence: { localMedian: baseline, rawPriceToken: raw, candidates, lookAheadPolicy: "prior_observations_only", asOfDate: input.reportDate }, proposal: this.createProposal({ proposedValue: String(best.value), proposedNumericValue: best.value, proposalMethod: "decimal_restoration", confidenceScore: confidence, reason: "Removing decimal precision best matches the robust prior-only sequence; review is required.", evidence: { selectedScale: best.scale, localError: best.error, lookAheadPolicy: "prior_observations_only", asOfDate: input.reportDate, resolutionEngineVersion: "data-quality-resolution-v1", resolutionWarnings: ["Sequence continuity can reflect legitimate corporate actions; manual review remains required."] } }) }];
  }
  ...
  detectImpossiblePercentage(input: QualityAppearanceInput): QualityFindingDraft[] {
    ...
    const evidence = { categoryName: input.categoryName, categoryType: input.categoryType, pennyContext: penny, marketCap: input.marketCap ?? null, threshold: suspiciousThreshold, rawPercentToken: raw, candidateDividedBy100: candidate, negativeMoveBelowMinus100: impossible, lookAheadPolicy: "same_day_source_only", asOfDate: input.reportDate };
