...
export async function processHistoricalDataQualityAudit(db: any, runId: string, limit = 250) {
  ...
  const engine = new HistoricalDataQualityEngine(), rows = new Map((evidence.data ?? []).map((row: any) => [String(row.id), row]));
  const results = items.map((item: any) => { const row: any = rows.get(String(item.appearance_id));if (!row) return { appearanceId: String(item.appearance_id), error: "Claimed appearance evidence was unavailable" };try { return engine.analyzeAppearance(toInput(row, nearest(byTicker.get(String(row.ticker_id)) ?? [], row))); } catch (error) { return { appearanceId: String(item.appearance_id), error: error instanceof Error ? error.message : String(error) }; } });
...
function nearest(sequence: SequenceObservation[], row: any) { const target = Date.parse(String(row.report_date));return sequence.filter(value => value.id !== String(row.id)).sort((a, b) => Math.abs(Date.parse(a.reportDate) - target) - Math.abs(Date.parse(b.reportDate) - target)).slice(0, 8); }
